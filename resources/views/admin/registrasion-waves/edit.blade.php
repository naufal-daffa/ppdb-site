...
@section('content')
    <div class="w-75 d-block mx-auto mt-3 p-4">
        ...
        <form action="{{ route('admin.registrasion-waves.update', $registrasionWave->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <input type="text" name="nama_gelombang" id="" class="form-control @error('nama_gelombang')
                    is-invalid
                @enderror" value="{{ $registrasionWave->nama_gelombang }}">
            ...
            <div class="mb-3">
                ...
                <input type="date" name="tanggal_mulai" id="" class="form-control @error('tanggal_mulai')
                    is-invalid
                @enderror" value="{{ $registrasionWave->tanggal_mulai }}">
                @error('tanggal_mulai')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <input type="date" name="tanggal_selesai" id="" class="form-control @error('tanggal_selesai')
                    is-invalid
                @enderror" value="{{ $registrasionWave->tanggal_selesai }}">
